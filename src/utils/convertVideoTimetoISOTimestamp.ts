 ...
 * 
 * @param startTime - The initial timestamp as a Date object
 ...
 */
export const convertVideoTimeToISOTimestamp = (startTime: Date, currentTime: number): string => {
  // Convert currentTime from seconds to milliseconds and add to start time
  const currentTimeMs = currentTime * 1000;
  const newTime = new Date(startTime.getTime() + currentTimeMs);
